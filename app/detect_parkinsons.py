...
if os.path.basename(args["dataset"])=='spiral':
	pickle.dump(model,open("model/spiral.h5",'wb')) #saving the spiral model
	np.save('model/spiral.npy',le.classes_)         #saving spiral label encoder

elif os.path.basename(args["dataset"])=='wave': 
	print(os.path.exists('model'))
	pickle.dump(model,open("model/wave.h5",'wb')) #saving the wave model
	np.save('model/wave.npy',le.classes_)         #saving wave label encoder
